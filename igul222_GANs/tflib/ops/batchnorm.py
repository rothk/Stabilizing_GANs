...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True):
    if ((axes == [0,2,3]) or (axes == [0,2])):
        ...
        if fused==False:
            ...
            if axes == [0,2]:
                return tf.transpose(result, [0,3,1,2])[:,:,:,0]
            else:
                return tf.transpose(result, [0,3,1,2])
        
        else:
            # Fused (fast but untested inference implementation):
        ...
        print('WARNING: old batchnorm implementation')
        # TODO use nn.fused_batch_norm here too for speedup
        mean, var = tf.nn.moments(inputs, axes, keep_dims=True)
        shape = mean.get_shape().as_list()
        if 0 not in axes:
            print('WARNING ({}): 0 not in axes'.format(name))
            shape[0] = 1
