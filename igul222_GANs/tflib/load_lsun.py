...
def make_generator(pathnames, n_files, batch_size, crop=True):
    ...
    def get_epoch():
        ...
        for n, i in enumerate(files):
            ...
            if crop:
                image = center_crop(image, 256, 256, 64, 64)
            else:
...
def load(batch_size, data_dir, crop=True):
    pathnames_train = glob(os.path.join(data_dir, 'bedroom_train/**/*.webp'), recursive=True)
    pathnames_val = glob(os.path.join(data_dir, 'bedroom_val/**/*.webp'), recursive=True)
    '''
    pathnames_train = []
    for path, dirnames, filenames in os.walk(os.path.join(data_dir, 'bedroom_train')):
        for filename in filenames:
            if filename.endswith('.webp'):
                pathnames_train.append(os.path.join(path, filename))
    pathnames_val = []
    for path, dirnames, filenames in os.walk(os.path.join(data_dir, 'bedroom_val')):
        for filename in filenames:
            if filename.endswith('.webp'):
                pathnames_val.append(os.path.join(path, filename))
    '''
    return (
